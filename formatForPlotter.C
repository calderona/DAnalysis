...
const TString histogram[nhistograms] = {"histoname1", "histoname2"};
...
void formatForPlotter() {
  ...
  TFile* rootfile = new TFile ("../output/outputfile.root", "read");
  ...
  for(int i=0; i < nprocess; i++) {
    ...
    for(int j=0; j < nhistograms; j++) {
      histo[i][j] = (TH1D*)rootfile -> Get(Form("d_%s/%s", process[i].Data(), histogram[j].Data()));
